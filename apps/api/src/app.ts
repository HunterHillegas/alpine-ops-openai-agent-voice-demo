...
import { createCompanyApi, delay, type CompanyApi } from "@alpine/company-api";
import { scenarios } from "@alpine/mock-data";

export function buildApiApp(options: { company?: CompanyApi; logger?: boolean } = {}) {
  const app = Fastify({ logger: options.logger ?? true });
  const company = options.company ?? createCompanyApi();
  ...
  app.post<{ Body: { customerId: string; assetId: string; priority: "low" | "normal" | "high" | "urgent"; summary: string; notes?: string[]; approvalToken: string } }>(
    "/tickets",
    async (request) => company.createTicket(request.body)
  );
  app.post<{ Body: { ticketId: string; status?: "open" | "triaged" | "scheduled" | "cancelled" | "resolved"; priority?: "low" | "normal" | "high" | "urgent"; summary?: string; note?: string; approvalToken: string } }>(
    "/tickets/update",
    async (request) => company.updateTicket(request.body)
  );
  app.post<{ Body: { ticketId: string; technicianId: string; windowId: string; reservedParts: string[]; customerChargeCents: number; approvalToken: string } }>(
    "/work-orders",
    async (request) => company.createWorkOrder(request.body)
  );
    ...
    const response = await fetch("https://api.openai.com/v1/realtime/client_secrets", {
      ...
      body: JSON.stringify({
        session: {
          ...
          model: realtimeModel.model,
          instructions: realtimeInstructions,
          audio: { output: { voice: realtimeModel.voice } }
        }
      })
    });
