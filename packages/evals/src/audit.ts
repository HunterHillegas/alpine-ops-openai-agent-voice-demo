...
export function runCompletionAudit(env: NodeJS.ProcessEnv = process.env): CompletionAudit {
  const checks: AuditCheck[] = [
    ...
    passed("mock-company-api", "Company API exposes resettable customers, assets, telemetry, warranty, policy, inventory, technicians, tickets, work orders, messages, notes, event log, shared typed client contracts, and route-level tests for read/write flows."),
    ...
    passed("replayable-scenarios", "Eval runner verifies twelve fixtures across routing, tool use, tool-call counts, approvals, exact entity capture, failure handling, approved writes, final mock state, and sent-message summary."),
